...
- [ ] **Step 1: Full Swift suite.** Expected `Executed 333 tests, with 0 failures` (use the whole-suite commands from Task 6 Step 7; re-run a lone `testPageAudioDittyStopsOnStopPageAudio` failure — known flake).
...
6. Connect to the home server. **In the Mac server terminal you should see** `synced storybook accepted for story … : 3 page(s), 0 image(s)` — and **no** rewrite starting for that story. The home Library shows the story `done` with the same title and pages. (`0 image(s)` is correct in Phase 1; pictures arrive in Phase 2.)
