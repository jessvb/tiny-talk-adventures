...
4. **Test script:**
   ...
   - **Cold-launch-hijack check (added after final review found this could reproducibly send the child to the wrong screen):** with at least one saved story already on the server, launch the app fresh and tap "Create a Story" → confirm you land on the live Story screen, not The End screen showing an old, unrelated story.
   - **Backgrounding-conclusion check (confirms the above fix didn't regress this pre-existing path):** conclude a story, background the app while its rewrite is still in progress, then foreground it again → The End should still appear for the story that just concluded.
   - **Rapid-re-tap audio check (added after final review found a potential audio-bleed bug):** on a Reading page, tap 🔊, then quickly swipe to a different page and tap 🔊 again before the first page's audio would have finished → confirm the new page's audio starts clean, with no tail end of the previous page's reading audible first.
   - **Disconnected-Library-tap check:** finish a story, tap Home (disconnecting), go to Read Stories → Library, tap a card → confirm the story opens normally rather than leaving Reading blank.
