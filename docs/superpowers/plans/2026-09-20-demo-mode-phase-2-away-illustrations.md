...
- [ ] **Step 1: Full Swift suite.** Expected `Executed 364 tests, with 0 failures` as originally written (362 on the originally planned 333 baseline) — **368** as executed (the review fixes added four tests; see "Post-execution corrections" at the end) (Task 3 Step 6's commands; re-run a lone `testPageAudioDittyStopsOnStopPageAudio` failure — known flake).
...
Expected on a clean transcription of this plan **as originally written**: no output — Phase 2's own files match the verified implementation. As actually executed, the output is exactly the four files with reviewed execution-time changes listed under "Post-execution corrections" at the end of this plan (`IllustrationPass.swift`, `ImageGeneration.swift` and their two test files) and nothing else; any other file in the output is a transcription difference to explain before opening the PR. (`AppModel.swift` is deliberately left out of this check: besides Phase 2's `makeIllustrator` it also carries Phase 1's reviewed execution-time changes, see "Post-execution corrections" at the end of Phase 1's plan.)
...
## Post-execution corrections (added 2026-09-21, after this plan was executed)

This plan was executed by a fresh subagent per task, each independently reviewed (spec + quality), followed by a whole-branch review and one fix wave with a scoped re-review. The resulting tree differs from the scratch implementation (`62b55d3`) that this plan's code blocks came from in exactly the reviewed ways below — four files (`IllustrationPass.swift`, `ImageGeneration.swift`, `IllustrationPassTests.swift`, `CloudflareImageClientTests.swift`); the other four files in Task 5 Step 3's check are byte-identical to it. Where they differ, trust the tree, not the blocks above.

1. **A 30 s per-request timeout (`a5fd859`).** `CloudflareImageClient` sets `request.timeoutInterval = 30`. URLSession's 60 s default equalled the whole illustration budget, and the budget is only checked *between* pages, so one stalled connection could run the pass a full minute over before anything noticed. A test asserts the built request carries it.
2. **The Cloudflare account id can no longer reach the debug log (`761ee56`, `e12e126`).** A `URLError`'s default description embeds the failing URL, which contains the account id — and a timeout is the *likely* failure on cellular — so the pass now logs errors as `URLError <code>: <localizedDescription>` (code and message only). Separately, Cloudflare's error 7003 body echoes the request path (it fires on a mistyped account id), so `.http(status:detail:)` replaces the id with `<account-id>` over the *whole* body and only then keeps the first 200 characters, so an id straddling the cut cannot survive. One test each.
3. **The 60 s time budget covers the drawing phase only (`11eba44`).** The spec and this plan say "a total time budget", and the code blocks above start the clock before the scene-sentence phase, so a slow Groq could cost every page its picture. The clock now starts just before the first picture is drawn. This follows the spec's stated purpose ("so a slow Cloudflare can't hold The End hostage"); the approved spec's wording was deliberately left unedited, pending the household's OK of a one-word amendment. One test (a slow scene phase still leaves the whole drawing budget).
4. **Credentials are trimmed at read time (`57921dc`, `6651e4d`).** A pasted trailing space or newline would have made every request fail with no obvious cause. `AppModel.makeIllustrator` trims both Cloudflare values and treats an empty result as missing (the pass is then not built); the Groq and animal-facts key reads in `connectAwayFromHome()` do the same.
5. **Debug-log fixes (`f57d17a`, `386353e`, `c81a05e`) and two tidy-ups (`0801d06`, `315d373`).** Every illustration debug line now carries the shared `[HH:mm:ss.SSS] ` prefix via a private `debug(_:)` helper — `AppModel.mergedDebugLog()` merges the logs with a plain string sort, so an unprefixed line sorted after everything else and this phase's only on-device instrument would have read out of order. An empty scene sentence now logs `illustration: page N scene prompt came back empty` (one test). A failed story sync now shows in the debug log as `demo story sync failed: …` rather than only being `print`ed. The two `try!` in the illustration tests became `try`, and two stale comments were refreshed.
6. **Counts.** Swift suite: **368** tests as executed (364 as written, plus the four tests above: HTTP-detail redaction, URLError redaction, slow scene phase, empty scene sentence). Server suite unchanged at 463. Task 5 Step 3's exactness check, as executed, reports the four files above and nothing else.
7. **The on-device script in Task 5 Step 5 was revised for the PR — use the PR's version.** It says to enter the credentials *before* connecting away from home (they are read when the away session starts), to paste one with a trailing space on purpose, to make two or three away stories before syncing, to read the now-timestamped log, and to confirm no account id appears anywhere in it.
8. **Known gaps deliberately not fixed here** (candidates for follow-up issues, not filed): no `Safety` check on the model-written scene sentence (the spec's accepted risk); an illustration pass interrupted by backgrounding or killing is never resumed (see the carry-overs at the top); `AVSpeechTts`'s "resolved name" debug line is still unstamped; `AppModel.appendAudioDebugEvent`'s doc comment still says it carries only the audio engine's lines; `disconnect()` clears that log, so a sync-failure line is lost if the socket dies at the same moment; the away-from-home toggle stays enabled for a whitespace-only Groq key (tapping it now says "no Groq API key saved"); the response shape of Cloudflare's FLUX endpoint is still unverified against a real response (the client accepts both the v4 envelope and a bare `{"image": …}`; Task 1 Step 7's `curl` check is pending with the household).
